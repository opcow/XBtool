...
// #ifndef DEBUG
	...
	m_WindowRect.left = max(rc.left,
		min(rc.right-w, m_WindowRect.left));
	m_WindowRect.top = max(rc.top,
		min(rc.bottom-h, m_WindowRect.top));
...
void CColourPopup::EndSelection(int nMessage)
{
    ...
    if (nMessage != CPN_SELENDCANCEL && m_nCurrentSel == CUSTOM_BOX_VALUE)
    {
        ...
        CMyColorDialog dlg(m_crInitialColour, CC_FULLOPEN | CC_ANYCOLOR, this);
...
void CColourPopup::OnActivateApp(BOOL bActive, DWORD ThreadID) 
{
	CWnd::OnActivateApp(bActive, ThreadID);
